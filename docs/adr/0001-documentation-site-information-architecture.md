...
- **Data display** — List & Tables (List [with ListRow, ListCol, TableHeader], DataTable, DataList), BarChart, Markdown
- **Typography** — Text
...
Reference         accessibility notes, the props table, and a live
                  ...
                  stories/MDX, surfaced here as `StoryEmbed`s under one or
                  more `###` subheadings, never as a bare top-level embed.
                  `### Playground` holds just the default `StoryEmbed`, and
                  only appears when no variant-like prop has any dedicated
                  story coverage. Otherwise the default embed moves under
                  that prop's own subheading (`### Variant`, `### Spacing`)
                  alongside its other covered values — `### Playground` and
                  a variant subheading never both appear (see rules 4–5).
```
...
2. **The heading vocabulary is closed.** No page invents new top-level sections ("Specifications", "HTML element"); new headings require updating the template first. `Variants`, `Accessibility`, and `Props` are retired as top-level Starlight headings — that material now lives under the single `Reference` heading, in Storybook. `Playground` survives, but only as a `###` subheading inside `Reference` (see rule 4), never as its own top-level section.
...
4. **Reference always shows the component live from Storybook, under a `###` subheading — and `### Playground` and a variant subheading never both appear on the same page.** If no variant-like prop (`variant`, `type`, `spacing`, ...) has any dedicated per-value story, Reference gets exactly one subheading, `### Playground`, holding the default `StoryEmbed`. As soon as a variant-like prop has _any_ dedicated story coverage — full or partial — the default embed moves under that prop's own subheading (`### Variant`, `### Spacing`) instead, alongside whatever other values are covered (see rule 5 for the partial case): the variant subheading already _is_ the playground, so a separate `### Playground` heading next to it would be redundant. `Menu` (`variant`, `spacing`) and `Tag` (`variant`) have full coverage, so neither has a `### Playground` heading at all; `ContentContainer` has partial coverage on `size`, so its default embed lives under `### Variant` next to the linked `wide` story, not under a separate `### Playground`; `FillButton` and `LayoutGrid` have no variant-like prop with any story coverage, so `### Playground` is their only Reference subheading.
5. **Partial story coverage gets linked, not embedded — but still lives under the variant subheading, alongside the default embed if the page has one.** If a variant-like prop has a dedicated story for only _some_ of its values, list and link those values (rather than embedding them; embedding a partial set risks implying it's the whole picture) under that prop's `### Variant`/`### Variants` subheading. `ContentContainer`'s `size` is covered for `medium` (the default story) and `wide`, but not `small`: the default embed goes under `### Variant` as usual, and `wide` is linked directly beneath it — there is no separate `### Playground`. The button family's `variant` is covered for `Primary`/`Secondary` only, on the shared `Button` overview story, which has no default story of its own to embed — so its `### Variant` section on the group page is links only. A value only reachable by changing a Control on a default story doesn't get an entry at all. Never write a variant list, or embed a variant grid, that Storybook itself can't back with a matching story per item.
   5a. **Two or more embeds stacked under the same subheading each get their own `####` label naming the specific value shown.** A bare stack of `StoryEmbed`s under one `### Variant`/`### Type`/`### State` heading forces the reader to guess which rendered example is which — a `Stroke` embed sitting next to a `Text` embed with no label reads as two unlabeled screenshots. Give each one its own `#### <value>` immediately above it (`#### Stroke`, `#### Text`; `#### General`, `#### Success`, …), matching `Menu`'s `#### Popover`/`#### Fixed` precedent. Skip the label only when a single embed already shows every value together in one canvas (`Anchor`'s `Modes` story renders all four variants side by side in one iframe) or when the subheading holds exactly one embed.
