...
export interface AbstractPropTransformer<T extends AbstractTheme> extends Prop<T> {
    prop: string;
...
export interface TransformFn<T extends AbstractTheme, P extends string, Config extends Prop<T>> {
    (value: Scale<T, Config>, prop: P, props: ThemeProps<T, {
        [K in P]?: Scale<T, Config>;
    }>): CSSObject;
}
export interface PropTransformer<T extends AbstractTheme, P extends string, C extends Prop<T>> extends AbstractPropTransformer<T>, Prop<T> {
    prop: P;
    styleFn: TransformFn<T, P, C>;
}
export declare type TransformerMap<T extends AbstractTheme, Config extends Record<string, Prop<T>>> = {
    [P in Key<keyof Config>]: PropTransformer<T, Key<P>, Config[P]>;
};
export declare type ParserProps<T extends AbstractTheme, Config extends Record<string, AbstractPropTransformer<T>>> = ThemeProps<T, {
    [P in keyof Config]?: Parameters<Config[P]['styleFn']>[2][Config[P]['prop']];
}>;
export interface Parser<T extends AbstractTheme, Config extends Record<string, AbstractPropTransformer<T>>> {
