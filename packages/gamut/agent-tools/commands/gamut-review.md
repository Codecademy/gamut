...
Use the appropriate `DESIGN.*.md` from `@codecademy/gamut/agent-tools` as the authoritative reference for the product's design intent, token names, and component patterns (`DESIGN.Codecademy.md`, `DESIGN.Admin.md`, `DESIGN.Platform.md`, `DESIGN.LXStudio.md`, or `DESIGN.Percipio.md`). YAML frontmatter in those files is generated from `@codecademy/gamut-styles` themes — run `yarn generate-agent-design-docs` after theme changes. When a finding maps to a skill, note it in the report so the developer knows where to get remediation guidance.
