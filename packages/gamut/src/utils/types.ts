 ...
 * too complex". This has broken real consumers three separate times
 * (`CheckboxProps`, `Alert`'s `cta`, `InfoTip`/`Disclosure`'s `buttonProps`),
 * always for someone downstream trying to narrow the prop bag, not for the
 * component itself.
 *
 ...
 * 2. If there's no safe key to omit, drop `DataAttributes` from that prop
 ...
 *
 * Safe without either fix: prop bags typed from a flat, non-union native
