...
describe('Icon', () => {
  it('renders an icon mapped by its name', () => {
    const iconName = 'bell';
    const wrapper = shallow(React.createElement(Icon, { name: iconName }));
  ...
  it('passes extra props to the icon', () => {
    ...
    const wrapper = shallow(
      React.createElement(Icon, Object.assign({ name: 'bell' }, extraProps))
    );
  ...
  it('adds an aria-label when a label prop is provided', () => {
    ...
    const wrapper = shallow(
      React.createElement(Icon, { label: label, name: 'bell' })
    );
    const iconProps = wrapper.find(iconMap.bell).props();
  ...
  it('sets the height and width when a size is provided', () => {
    const size = 117;
    const wrapper = shallow(
      React.createElement(Icon, { name: 'bell', size: size })
    );
    const iconProps = wrapper.find(iconMap.bell).props();
    expect(iconProps).toMatchObject({
      height: size,
