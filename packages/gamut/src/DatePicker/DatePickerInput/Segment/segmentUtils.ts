 ...
 */
export const parseSegmentsToDate = (segments: SegmentValues): Date | null => {
...
/** Clamp and pad segments after blur (digits only). */
export const normalizeSegmentValues = (s: SegmentValues): SegmentValues => {
  const year = s.year.replace(/\D/g, '').slice(0, 4);
  let month = s.month.replace(/\D/g, '').slice(0, 2);
  let day = s.day.replace(/\D/g, '').slice(0, 2);
...
export const parseSegmentNumericString = (s: string): number | null => {
...
const parseSegmentDigits = (_field: DatePartKind, s: string): number | null =>
  parseSegmentNumericString(s);

const padSegmentNumber = (field: DatePartKind, n: number): string => {
  if (field === 'year') {
    const clamped = Math.min(9999, Math.max(1, n));
...
export const appendSegmentDigit = (
  ...
  return (prev.replace(/\D/g, '') + digit).slice(0, maxLen);
};
...
export const buildCombinedFromSegments = (
  ...
  layout
    .map((item) =>
      item.kind === 'literal' ? item.text : segments[item.field]
    )
    .join('');
