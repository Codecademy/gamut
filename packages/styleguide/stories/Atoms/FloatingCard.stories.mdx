...
The wrapper may need to be modified, such as for the `display` to be set to `block` instead of the default `inline-block`
...
export const MyComponent: React.FC = () => {
  return (
    <FloatingCard pattern={DotDense} cardWrapperLayout={{ display: 'block' }}>
