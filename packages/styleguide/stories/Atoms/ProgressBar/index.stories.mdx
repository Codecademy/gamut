...
ProgressBar comes in one of three sizes: `small`, `medium`, and `large`.
      ...
      <LayoutGrid columnGap="sm" rowGap="sm">
        {[0, 25, 50, 75, 100].map((percent) =>
          bars.map(({ size, variant }) => (
            <Column key={[percent, size, variant].join()} size={4}>
...
Using the new SVG Pattern component for the background. Here we are using the DiagonalStripeBRegular Pattern in navy in the large ProgressBar and the DiagonalStripeBDense pattern in the smaller ProgressBar.
...
<Canvas column={2}>
  <Story
    name="Stripe - Regular"
    args={{
      pattern: 'DiagonalStripeBRegular',
      size: 'large',
      percent: 50,
      variant: 'yellow',
    }}
