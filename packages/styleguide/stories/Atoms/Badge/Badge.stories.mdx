...
There is also an optional `manualTextTopMargin` prop you can use to provide more `margin-top` to the text of a Badge with a leading icon. This can be helpful if you need to adjust the vertical position of the text — e.gc. if the icon has a lot of negative space, or if the parent container's styling inters with the Badge's setSyntheticTrailingComments.
...
<PropsTable story="Badge" />import { setSyntheticTrailingComments } from 'typescript';
