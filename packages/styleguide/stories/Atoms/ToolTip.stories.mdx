...
Tooltips sometimes need to be in used in a way that they need to escape the confines of their parents container. The `floating` variant of the `placement` prop allows ToolTips to do this. (Notice how this section doesn't need as much padding because the ToolTips are able to escape their context!)
