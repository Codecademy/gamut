import { Box, FlexBox, ToolTip } from '@codecademy/gamut/src';
...
_Note:_ Because of the size of these ToolTips, they should never overflow their container so are not applicable to `isPopover` Tooltips.
...
Tooltips sometimes need to be in scrollable content, and that's okay!

<Canvas>
  <Story name="isPopover">
    {(args) => {
      return (
        <FlexBox center bg="paleBlue" p={24}>
          Updog <ToolTip isPopover {...args} />
          Updog <ToolTip isPopover id="42">
            hi cassie{' '}
          </ToolTip>
          Updog{' '}
          <ToolTip isPopover id="42" target={<InfoCircleIcon size={70} />}>
            hi cassie{' '}
          </ToolTip>
          Updog <ToolTip
            isPopover
