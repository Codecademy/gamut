...
GlobalHeader takes in several props.`type` determines which header to render based on the user role (anonymous, free, or pro). `type` will also determine which of Search, Notification, and Profile components need to be passed in: Search will appear for all users, whereas Notifications and Profile will only appear for logged-in users. `onClick` will be defined by the consumer and handle any routing/tracking that needs to be done.
