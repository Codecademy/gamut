...
You might be implementing an interaction that needs to be aware of the `GlobalHeader` height (for example when using [window.scrollTo](https://developer.mozilla.org/en-US/docs/Web/API/Window/scrollTo)). `gamut-labs` exports a `useGlobalHeaderHeight` hook for this purpose:

```jsx
import { useGlobalHeaderHeight } from 'gamut-labs';
