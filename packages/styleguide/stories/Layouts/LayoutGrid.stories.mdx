...
<Meta
  ...
  argTypes={{ columns: { table: { disable: true } } }}
  args={{ gap: 16, columns: [12, 6, 6, 3, 3, 3, 3], rowHeight: '8rem' }}
/>
...
<Canvas>
  <Story name="LayoutGrid">
    {({ columns, ...args }) => (
      <LayoutGrid {...args}>
        {columns.map((col, i) => (
          <Column key={`${col}-general`} size={col} rows={(i % 2) + 1}>
            <ExampleBox>{col}</ExampleBox>
          </Column>
        ))}
      </LayoutGrid>
    )}
