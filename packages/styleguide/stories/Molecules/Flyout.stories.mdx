import { Box, FlexBox, TextButton } from '@codecademy/gamut/src';
...
<Meta
  ...
  parameters={{
    subtitle:
      'Flyout is a general container that is mobile/tablet menu navigation. On button click, a container animates in from the left side of the window. It can be configured to close by pressing the "X" close icon, pressing they "esc" key, or using the `onToggle` callback prop for customizability.',
    source: 'gamut-labs',
    status: 'updating',
  }}
/>
...
<Canvas>
  <Story name="Flyout">
    {(args) => (
      <Box height="20rem">
        <Flyout openFrom="left" button={<TextButton>chiller time</TextButton>}>
