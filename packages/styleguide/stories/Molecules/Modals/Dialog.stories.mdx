...
import { ClickOpenDialog } from './Dialog.examples';
...
You can also cause a Dialog to appear/disappear by maintaining a state for `isOpen`. The example below toggles `isOpen` with a button click.

<Canvas>
  <Story name="Trigger Dialog">
    {() => {
      return <ClickOpenDialog />;
    }}
  </Story>
</Canvas>
