...
import { ClickOpenModal } from './Modal.examples';
...
You can also cause a Modal to appear/disappear by maintaining a state for `isOpen`. The example below toggles `isOpen` with a button click.

<Canvas>
  <Story name="Trigger Modal">
    {() => {
      return <ClickOpenModal />;
    }}
  </Story>
</Canvas>
