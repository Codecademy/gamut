...
- `Dialog`: Structured modals with binary user interaction paths (Confirm / Cancel).
- `Toast`: Low priority decorative and celebratory user feedback.
- `Coachmark`: Highlight a desired interaction or new feature.
- `Alert`: Actionable direct user feedback.

#### Subcomponents

- `Overlay`: Accessible and content agnostic screen takeovers.
- `BodyPortal`: Escaping the current DOM context.
- `FloatingCard`: Cards with pattern drop shadows.
