import { Disclosure } from '@codecademy/gamut/src/Disclosure';
import title from '@codecademy/macros/lib/title.macro';
import { PropsTable } from '@codecademy/storybook-addon-variance';
import { Canvas, Meta, Story } from '@storybook/addon-docs/blocks';

// TO ASK: why does this need `.../src/Disclosure` when other imports don't?
...
<Meta
  ...
  parameters={{
    subtitle:
      'A Disclosure is an expandable container featuring a header that serves as a toggle to expand or collapse a content section.',
    source: 'gamut',
    status: 'current',
    padded: true,
  }}
...
`spacing` is an optional prop you can pass to `<Disclosure>`. By default it's set to `"normal"`, though you can set it to `"condensed"` or `"compact"` as well. Spacing will determine the height of the header, the size of the chevron icon, as well as the padding of both the header and the body.
...
- `overline` can be given a string value to appear above the header
- `subheader` can be given a string value to appear below the header
...
- `widthBackground`: is defaulted to `false` but if assigned `true` will render an background with some padding for the text.
- The Disclosure can be set to `disabled` and rendered unclickable
  CHANGE THE LINE BELOW ONCE THE BUTTON LOGIC IS FIXED
...
<Canvas>
  <Story
    ...
    args={{
      ...
      subheader: 'subheader is text-secondary',
