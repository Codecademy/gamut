...
<Canvas>
  <Story
    ...
    args={{
      ...
      body:
        'Notice the LACK of border and it uses `background-current`. Set `variant="block"` to enable this styling.',
      variant: 'block',
    }}
  >
    {(args) => <Disclosure {...args} />}
...
When creating a List of Disclosures, make sure to set `isListItem` to `true`.
<Canvas>
...
### Using `<Background>` with Disclosure
You can nest Disclosure inside a `<Background>` to provide a different color.
<Canvas>
  <Story name="just for testing">
    <Background bg="background-primary">
    <Disclosure
        heading="Hint"
        initiallyExpanded={false}
        body="possible mock for a hint, slight concern about spacing though... since all this text will get smushed up, if that's ok? then I can work a little more on this. Maybe the only thing that really needs to be exposed is an expandedTitle and a collapsedTitle"
        href="/"
        spacing="compact"
      />
  </Background>
