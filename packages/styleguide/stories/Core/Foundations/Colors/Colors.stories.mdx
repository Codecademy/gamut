import { startCase } from 'lodash';
import {
  StoryStatus,
  Swatch,
  SwatchPalette,
  parseCamelCase,
  objectKeys,
} from '~styleguide/blocks';
import { Meta, Props, Canvas, Story } from '@storybook/addon-docs/dist/blocks';
import {
  LayoutGrid,
  Column,
  VisualTheme,
  Container,
  Heading,
} from '@codecademy/gamut/src';
import {
  brandColors,
  colors,
  swatches,
  editorColors,
  platformColors,
} from '@codecademy/gamut-styles';
import { excludedColors } from './constants';

<Meta title="Foundations/Colors" component="Colors" />

# Colors

<StoryStatus status="stable" />

These are the color palettes used by both design and engineering.

## Common Color Sets

Standard color atoms we select from in creating designs. All colors on
Codecademy properties should adhere to these. Remember your accessibility
contrast requirements too!

### Usage

```tsx
import { colors } from '@codecademy/gamut-styles';

colors.standard.blue;
colors.background.lightBlue;
```

```scss
@import '@codecademy/gamut-styles/utils';

.standardRed {
  color: $standard-red;
  background-color: $background-light-blue;
}
```

### Standard Colors

<Story name="Standard">
  <LayoutGrid rowGap="md" columnGap="md">
    {objectKeys(colors.standard).map((color) => (
      <Column key={color} size={{ xs: 6, sm: 3 }}>
        <Container flex={false}>
          <Heading as="h2" fontSize="xs">
            {startCase(color)}
          </Heading>
          <Swatch
            name={`$color-standard-${parseCamelCase(color)}`}
            hex={colors.standard[color]}
          />
        </Container>
      </Column>
    ))}
  </LayoutGrid>
</Story>

<br />

### Background Washes

<Story name="Light Backgrounds">
  <LayoutGrid rowGap="md" columnGap="md">
    {Object.keys(colors.background.light).map((color) => (
      <Column key={color} size={{ xs: 6, sm: 3 }}>
        <Container flex={false}>
          <Heading as="h2" fontSize="xs">
            {startCase(color)}
          </Heading>
          <Swatch
            name={`$color-background-light-${parseCamelCase(color)}`}
            hex={colors.background.light[color]}
          />
        </Container>
      </Column>
    ))}
  </LayoutGrid>
</Story>

<br />

### Interactive Colors

For usage on links, buttons, and other interactive elements on dark and light backgrounds

<Story name="Interactive Colors">
  <LayoutGrid rowGap="md" columnGap="md">
    {Object.keys(colors.interactive).map((color) => (
      <Column key={color} size={{ xs: 6, sm: 3 }}>
        <Container flex={false}>
          <Heading as="h2" fontSize="xs">
            {startCase(color)}
          </Heading>
          <Swatch
            name={`$color-interactive-${parseCamelCase(color)}`}
            hex={colors.interactive[color]}
          />
        </Container>
      </Column>
    ))}
  </LayoutGrid>
</Story>

<br />

### Base Colors

<Story name="Base">
  <LayoutGrid rowGap="md" columnGap="md">
    {['black', 'white'].map((color) => (
      <Column key={color} size={{ xs: 6, sm: 3 }}>
        <Container flex={false}>
          <Heading as="h2" fontSize="xs">
            {startCase(color)}
          </Heading>
          <Swatch
            name={`$color-${parseCamelCase(color)}`}
            hex={colors[color]}
          />
        </Container>
      </Column>
    ))}
  </LayoutGrid>
</Story>

<br />

## Swatches

Full-color swatches.

### Usage

```tsx
import { colors } from '@codecademy/gamut-styles';

colors.blue[900];
```

```scss
@import '@codecademy/gamut-styles/utils';

.darkBlue {
  color: $color-blue-900;
}
```

<Story name="Rest">
  {(args) => (
    <LayoutGrid rowGap="md" columnGap="md">
      {objectKeys(swatches)
        .filter((color) => !excludedColors.includes(color))
        .map((color) => (
          <Column key={color} size={{ xs: 6, sm: 3 }}>
            <Container flex={false}>
              <Heading as="h2" fontSize="xs">
                {parseCamelCase(`color-${color}`)}
              </Heading>
              <SwatchPalette
                variablePrefix={`color-${color}`}
                data={swatches[color]}
              />
            </Container>
          </Column>
        ))}
    </LayoutGrid>
  )}
</Story>

## Platform Colors (Learning Environment)

Our Learning Environment ("LE") is, for now, an exception to the rule
about containing only brand and grid colors. It uses a selection of muted
grayish purples and a few mid-range minty greens.

<Story name="Platform ">
  {(args) => (
    <LayoutGrid rowGap="md" columnGap="md">
      {objectKeys(platformColors).map((color) => (
        <Column key={color} size={{ xs: 6, sm: 3 }}>
          <Container flex={false}>
            <Heading as="h2" fontSize="xs">
              Platform {startCase(color)}
            </Heading>
            <SwatchPalette
              variablePrefix={`platform-${color}`}
              data={platformColors[color]}
            />
          </Container>
        </Column>
      ))}
    </LayoutGrid>
  )}
</Story>

<br />

## Editor Colors

The LE's code editor uses its own colors for text.

<Story name="Editor">
  {(args) => (
    <LayoutGrid rowGap="md" columnGap="md">
      {objectKeys(editorColors).map((color) => (
        <Column key={color} size={{ xs: 6, sm: 3 }}>
          <Container flex={false}>
            <Heading as="h2" fontSize="xs">
              Editor {startCase(color)}
            </Heading>
            <Swatch
              name={`color-editor-${parseCamelCase(color)}`}
              hex={editorColors[color]}
            />
          </Container>
        </Column>
      ))}
    </LayoutGrid>
  )}
</Story>

<br />

## Brand Colors (deprecated)

<Story name="Brand">
  <LayoutGrid rowGap="md" columnGap="md">
    {objectKeys(brandColors).map((color) => (
      <Column key={color} size={{ xs: 6, sm: 3 }}>
        <Container flex={false}>
          <Heading as="h2" fontSize="xs">
            {startCase(color)}
          </Heading>
          <Swatch
            name={`$brand-${parseCamelCase(color)}`}
            hex={brandColors[color]}
          />
        </Container>
      </Column>
    ))}
  </LayoutGrid>
</Story>
