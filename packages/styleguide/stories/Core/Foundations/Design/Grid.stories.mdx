...
<Meta
  title="Foundations/Design Guidelines/Grids"
...
We have 6 main screensizes we design for, these are not the same thing as `breakpoints` which are used to help us achieve these guidelines. You can checkout our [breakpoints here](?path=/docs/foundations-theme--page#breakpoints)
...
The designer decides how many columns at each breakpoint a section should span. The grid will allow the engineer to set a minimum amount of columns for each breakpoint to avoid having. For more information checkout [responsive properties](/?path=/docs/foundations-system-props--page#responsive-properties)

```tsx
// Object syntax
<Column size={{ base: 6, xs: 4 }}>Content</Column>
...
For spacing between components and sections use these as a guide. Each of these scales is a subset of our `spacing` sizes found on [our theme](?path=/docs/foundations-theme--page#spacing). While you will have access to all of them in your code use these sets as a practical guide.
