...
There are many cases where you may need a specifc background color for a section of a page, like a card or a landing page. Instead of having to guess the right mode for the background, we've added a `<Background />` component to automatically detect the contrast of the background and change the mode to an acessible one. Allowing you to be sure that all components inside your background will meet contrast requirements and designs without any configuration!
