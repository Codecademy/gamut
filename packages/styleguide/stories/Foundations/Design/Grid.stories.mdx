...
import { Box, DataTable } from '~styleguide/blocks';

<Meta
  title={title}
  parameters={{
    subtitle: `Rules for how pages should be layed out and how we handle different viewports`,
      ...
      render: ({ sizes }) => (
        <>
          ...
          >
            ...
            {sizes.map((size) => (
              <Box key={`value-${size}`} textAlign="center" fontSize={'12px'}>
                {size}
              </Box>
            ))}
          </Box>
