...
There may be times where must specify a specifc background color. We've added a `<Background />` component to detect the contrast requirements of the current color mode, and detect if it needs to be changed. Allowing you to be sure that all components inside your background will meet contrast requirements and designs without any extra configuration.
