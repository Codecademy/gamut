...
- `scrollbar`: The color used for the scrollbar thumb at the current level.
...
When you use `Background` the scrollbar track will change color to match whatever color is set as `background-current`. Nested `ColorModes` and containers with `Background` will use the closest `background-current` variable available. Scrollbar thumb colors are controlled in individual `Theme` objects.
...
- `useColorModes()`: If you need access to all modes and their variables this hook will return a tuple of `[mode, modeColors, modes]`.
...
const [current, currentColors, modes] = useColorModes();
