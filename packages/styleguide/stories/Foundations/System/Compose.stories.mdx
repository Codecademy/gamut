...
When using custom properties or our standard <LinkTo id="foundations-system-props--page">system props</LinkTo> it can be useful to combine them into a single function using `compose`.
...
Using `compose` will take any number of system prop functions and merge them left to right (the last argument takes precedence if props conflict) and returns a new prop function with the intersection of these pieces.

```tsx
import { compose } from '@codecademy/variance';
import { layout, grid } from '@codecademy/gamut-styles';

const Grid = styled.div(compose(layout, grid));
