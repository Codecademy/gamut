...
This like `variant` accepts an object allows you to specify any number of style configurations, however instead of being limited to a single prop, each set of styles can be enabled concurrently. Each key specified on the configuration object becomes a boolean prop type - allowing you to enable / disable specific behaviors as needed.
