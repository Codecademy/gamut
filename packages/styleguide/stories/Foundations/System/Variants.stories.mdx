...
`states` accepts an object of keyed CSS objects. Unlike `variant` each key represents an individual boolean prop ( instead of options of a single prop) that can be simultanerously configured. This makes enabling / disabling specific behaviors as needed far easier as they do not need to be reduced to a single mutually exclusive set of states.
