...
import { LayoutMenuVariant } from '@codecademy/gamut-labs/src';
...
<Meta
  title={title}
  component={LayoutMenuVariant.LayoutMenu}
  args={{
    sections: [
      {
        title: 'Languages',
...
<Canvas>
  <Story name="LayoutMenu">
    {(args) => <LayoutMenuVariant.LayoutMenu {...args} />}
  </Story>
</Canvas>
...
Additional links outside of an accordion can be added as `children`. You can use the `SectionItemLink` component (pass an `onClick` prop and link text as `children`) to match the styling of the accordion items.

<Canvas>
  <Story name="With Additional Links">
    {(args) => (
      <LayoutMenuVariant.LayoutMenu {...args}>
        <Box py={16}>
          <LayoutMenuVariant.SectionItemLink
            onClick={() =>
