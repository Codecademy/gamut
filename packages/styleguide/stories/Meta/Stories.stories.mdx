...
We've provided a few helpful vscode snippets to help you get through boilerplate. To use these simply start to type these strings in your editor and pick the template and fill out the tab targets.
...
- The folder struture is identical to the storybook heirarchy generated.

First find the right folder for your story in `packages/styleguide/stories` (e.g. `Atoms` | `Molecules` | `Organisms`).Once you've found it create a new file `#{COMPONENT_NAME}.stories.mdx` (if you need an adjacent file to store variables create a folder of the component name and create an `index.stories.mdx` inside of it).
...
Your new story will be generated with the correct heirarchy and added to storybook!
...
3. `status`: The health of the components API (e.g. `stable` | `volatile` | `deprecated`)
