...
Each section of stories, such as `Atoms` and `Labs`, contains a high-level table summarizing the stories inside it.
When adding a story, be sure to add a corresponding entry in its section's `constants.tsx`.
