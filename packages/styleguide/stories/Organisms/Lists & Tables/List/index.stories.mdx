...
Our `List`s by default are the `ul` HTML element or unordered list. All of the above configurations works with ordered lists as well. Passing `ol` to the `as` prop will render an ordered list and add numbering to the rows. Ordered lists should always have aheader

<Story name="Ordered List" args={{ as:"ol",  variant: 'table', spacing: 'condensed' }}>
  {DemoTemplate.bind({})}
</Story>
...
- `header`: The column of the row with any critical indifying information goes at the top of the view. This is especially important for ordered lists as this is where the numbering will render.
...
<PropsTable story="List" exclude={['overflowHidden']} />import { list } from '@codecademy/gamut-styles/dist/variance/props';
