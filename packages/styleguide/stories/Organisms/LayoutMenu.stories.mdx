...
Additional links outside of an accordion can be added as `children`. You can use the `MenuItem` component (pass an `onClick` prop and link text as `children`) to match the styling of the accordion items. You can also use the `active` prop is.

<Canvas>
  <Story name="With Additional Links">
    {(args) => (
      <LayoutMenu {...args}>
        <Menu variant="navigation" minWidth="max-content" border="none" pb={32}>
          <MenuItem px={4} active>
            Full Catalog
          </MenuItem>
        </Menu>
        <Menu variant="navigation" minWidth="max-content" pb={32} border="none">
          <MenuItem
            px={4}
            onClick={() =>
