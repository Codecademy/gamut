...
<Meta
  ...
  args={{
    ...
    onSectionItemClick: (item) => {
      // eslint-disable-next-line no-console
      console.log(
        `You should handle where the link goes here and also could add event tracking. You have access to the menu item: ${JSON.stringify(
          item
        )}`
      );
    },
...
Disable showing the Codecademy logo at the top of the Flyout by not passing the `showLogoOnFlyout` prop, or passing it as `false`.
