...
The GridForm organism provides an easy, out-of-the-box way to implement forms from a list of fields. When provided a list of fields, GridForm strings together the appropriate [Form Elements](/?path=/docs/atoms-formelements-about--page) inside a [LayoutGrid](/?path=/docs/layouts-layoutgrid--layout-grid).
...
- **Sections**: Contains the headers and dividers that are rendered in the optional [Sections](?id=organisms-gridform--submit-button-options&viewMode=docs#sections) modules
- **Inline Submit**: A form incorporating the [Inline Submit](?id=organisms-gridform--submit-button-options&viewMode=docs#inline-submit-button) Button style of GridForm
...
We can use the `size` and `rowspan` props (borrowed from [LayoutGrid](/?path=/docs/layouts-layoutgrid--layout-grid)) to customize the layouts of our GridForms.
...
You can set the [Text variant](https://gamut.codecademy.com/storybook/?path=/docs/typography-text--text) prop for the section title the same way. Only title variants are accepted.
