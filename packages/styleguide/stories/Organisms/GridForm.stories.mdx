...
import { ColorModeExample, FormLoadingExample } from './examples';
...
You can set the `breakType` prop to adjust the type of separation between each section.

The following values can be passed to `breakType`:

- `none`: no border
- `default`: 1px solid border that matches color of `text`

<Canvas>
  <Story name="Section Breaks">
    {(args) => {
      return (
        <GridForm
          breakType="none"
...
`disableFieldsOnSubmit` will disable all form fields once the GridForm has been successfully submitted. If you have any server validation that needs to happen, we reccommend using the `wasSubmitSuccessful` prop, but submission will also fail if a promise is rejected within your `onSubmit` or if a field does not pass validation.
...
`resetOnSubmit` will reset the form once the GridForm has been successfully submitted. If you have any server validation that needs to happen, we reccommend using the `wasSubmitSuccessful` prop, but submission will also fail if a promise is rejected within your `onSubmit` or if a field does not pass validation.
...
We can combine these together to create some pretty cool loading state, forms which disable their fields while submitting, and reset the form when the submit was successful.
