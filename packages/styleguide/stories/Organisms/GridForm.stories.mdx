...
<Meta
  ...
  component={GridForm}
  parameters={{
    subtitle: `GridForm is the most efficient way to build and design forms at Codecademy.`,
    source: 'gamut',
    status: 'updating',
    figmaId: 'ReGfRNillGABAj5SlITalN/%F0%9F%93%90-Gamut?node-id=1689%3A3910',
  }}
/>
...
While `Form` atoms compose the structure of these forms, we strongly recommend only creating forms with using `GridForm`. You should only use the form element atoms as individual elements if your interface doesn’t need to support the technical functions of a form.
...
We can use the `size` and `rowspan` props (borrowed from <LinkTo id="layouts-layoutgrid--layout-grid">LayoutGrid</LinkTo>) to customize the layouts of our GridForms.
...
A field can include <LinkTo id="atoms-tooltip--tool-tip">our existing `ToolTip`</LinkTo>. The position of the tooltip on each field is always set to the bottom-right. You also have the option to render each tooltip in dark or light mode.

See the <LinkTo id="atoms-forminputs-radio--radio">`Radio`</LinkTo> story for an example of how to add a tooltip to a radio option.
...
You can set the <LinkTo id="typography-text--variants">Text variant</LinkTo> prop for the section title the same way. Only title variants are accepted.
...
TO-DO: FILL OUT DOCS. CASS - You need to fill these docs out! the size of each field needs to be 6 or it's gonna be really wonky and each column rowspan needs to add up to 12 (including the title! to make them flow correctly.) so if you have 4 questions in each section and you have 2 sections, the last question should be 2 or something.

<Canvas>
  <Story name="Column GridForm">
    {(args) => (
      <GridForm
        columnFlow
        fields={[
