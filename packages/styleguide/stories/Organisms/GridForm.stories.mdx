...
Each sections should have a title that correctly follows [heading ranks](https://usability.yale.edu/web-accessibility/articles/headings). You can set the text type of this title though the `as` prop on that section -- the default is `h2`.

<Canvas>
  <Story name="Custom Sections">
    {(args) => {
      return (
        <GridForm
          fields={[
...
We call it a "sweet container" so that bots do not immediately detect it as a honeypot input. test
