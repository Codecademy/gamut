...
`<Text />` is the preferred component for customizing typography. The API is progressively specific. In order to achieve the effect you are looking for there are 3 levels of precedence.
...
<Text as="h4" variant="title-1" />;
```

**Note**: Be aware that you must replicate any default behavior if you override these props with responsive values.
