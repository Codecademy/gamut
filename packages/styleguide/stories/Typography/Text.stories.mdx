...
### Line Truncation (default: 0 aka no truncation )

<Canvas>
  <Story name="Truncation">
    <LayoutGrid gap={32}>
      {typographyUtilities.truncation.map((lines) => (
        <Fragment key={lines}>
          ...
          <Column size={9}>
            <Text truncateLines={lines} width="400px">
              This is a very long text thingy that we want to truncate it's very
              long indeed. This is a very long text thingy that we want to
              truncate it's very long indeed. This is a very long text thingy
              that we want to truncate it's very long indeed. This is a very
              long text thingy that we want to truncate it's very long indeed.
            </Text>
          </Column>
        </Fragment>
      ))}
