...
import { MiniInfoOutlineIcon, MiniOpenIcon } from '@codecademy/gamut-icons';
...
import {
  InlineAnchorWithIconAndText,
  VariantsExample,
} from './Anchor.examples';
...
<Story name="modes">{() => <VariantsExample useIcon={false} />}</Story>
...
<Canvas>
  <Story name="Anchor with Icon">
    {(args) => <InlineAnchorWithIconAndText args={args} />}
  </Story>
</Canvas>

Here's how icons can look with all the variants.

<Story name="modes with icons">{() => <VariantsExample useIcon />}</Story>
...
<Canvas>
  <Story name="Anchor">
    {(args) => {
      return (
        <>
          <Anchor {...args} icon={MiniOpenIcon} iconPosition="right">
            I am a cool link
          </Anchor>{' '}
        </>
      );
