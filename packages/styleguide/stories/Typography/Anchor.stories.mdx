import { Anchor, Box, Column, FlexBox, LayoutGrid, Text } from '@codecademy/gamut/src';
import { MiniArrowLeftIcon, MiniArrowRightIcon, MiniInfoOutlineIcon } from '@codecademy/gamut-icons';
...
## Adding an Icon
You can use an <LinkTo id="atoms-icons-about--page">Icon</LinkTo> can be added to the right or left of the Anchor text.
* Use the `icon` prop to specify the Icon
* set `iconPosition` to `"left"` or `"right"` for placement (default is `"left"`)
* set `center` to `true` to horitzonally center the icon and text
  * the Icon and text come vertically centered
    ...
    {(args) => {
      return (
        ...
        <FlexBox justifyContent="space-between">
          <Anchor {...args} href="/" icon={MiniArrowLeftIcon} iconPosition="left" variant="inline">Previous</Anchor>
          <Anchor {...args} href="/" icon={MiniArrowRightIcon} iconPosition="right" variant="inline">Next</Anchor>
        </FlexBox>
        <Text>
        <Anchor {...args} href="/" icon={MiniInfoOutlineIcon} iconPosition="left" variant="inline">Centered</Anchor> Testing
        </Text>
        </>
      )
    }}
