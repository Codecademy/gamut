...
import * as ResponsivePropsStories from './ResponsiveProperties.stories';
...
<TokenTable
  {...{
    ...breakpoint,
    rows: breakpoint.rows.filter(({ id }) => id.startsWith('c_')),
  }}
/>
...
Array indices extend after the viewport breakpoints:

```
[base, xs, sm, md, lg, xl, c_xs, c_sm, c_md, c_lg, c_xl]
```

```tsx
<Box color={['red', , , , , , 'blue', , 'purple']} />
```
...
### Mixing media & container queries

```tsx
<Box display={{ _: 'block', lg: 'flex', c_md: 'grid' }} />
```
...
<Story of={ResponsivePropsStories.MediaVsContainer} />
