...
const styles = styled.div`
  blue: ${({ theme }) => theme.colors.teal}; // theme here refers to the current theme, which is the platform theme
`;
```
...
const notWorkingStyles = css`
  color: ${theme.colors.teal}; // teal does not exist in the Core Theme colors so this does not work
`;
```
