...
Set `multiple` to let users choose more than one option. Selected options render as removable tags in the input, and `onChange` receives the full array of selected options (each as `{ label, value }`) rather than a single value. Manage that array as a [controlled component](https://reactjs.org/docs/forms.html#controlled-components).
...
Two behaviors come along automatically in creatable mode:

- **Searchable by default** — `isSearchable` is forced to `true`, since creation needs a text input. Passing `isSearchable={false}` alongside `isCreatable` is a TypeScript error.
- **Typed text persists on blur** — if the user clicks away or the menu closes mid-entry, their text is kept so they can pick up where they left off. It clears only once an option is selected or created.
...
Combine `isCreatable` with `multiple` to let users select several existing options and add their own. Each created value is appended to the options list and included in the current selection.
