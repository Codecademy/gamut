...
The component that expands the `Drawer` should have an `aria-expanded` and `aria-controls` attribute. The `aria-controls` attribute should point to the `id` of the `Drawer`. The `aria-expanded` attribute should be set to the value of the `expanded` prop passed to the `Drawer`.
...
const ExampleDrawer: React.FC = () => {
  ...
  return (
    <FlexBox >
      <Drawer expanded={expanded} id={drawerId}>Example Drawer</Drawer>
      <StrokeButton
