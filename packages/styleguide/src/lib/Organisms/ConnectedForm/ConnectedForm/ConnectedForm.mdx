...
`ConnectedForm` is an organism designed to create flexible forms with all of the reliability of <LinkTo component='Organisms/GridForm'>GridForm</LinkTo> but without the strict grid structure.

### Components

- `ConnectedForm`: A <LinkTo component='Atoms/Form'>Form</LinkTo> that provides context to all `Connected` components.
- `ConnectedFormGroup`: <LinkTo component='Atoms/FormGroups'>FormGroups</LinkTo> that use the context provided by `ConnectedForm` for error and disabled states.
- `ConnectedFormInputs`: <LinkTo component='Atoms/FormInputs'>FormInputs</LinkTo> the use the context provided by `ConnectedForm` for field registration and validation as well as required and disabled states. Should be used through `ConnectedFormGroup`.
- `SubmitButton`: A submit type <LinkTo component='Atoms/Button'>Button</LinkTo> that uses the context provided by `ConnectedForm` for loading and disabled states.
