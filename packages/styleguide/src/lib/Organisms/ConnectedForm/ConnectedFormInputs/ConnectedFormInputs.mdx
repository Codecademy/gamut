...
We have a selection of `ConnectedInput` components that are visually identical to their un-connected <LinkTo component='Atoms/FormInputs'>FormInput</LinkTo> counterparts. Please reference the links below for component-specific prop and styling configuration:
...
For further styling configurations, check out <LinkTo component="Atoms/FormInputs/RadioGroup">RadioGroup</LinkTo>.

`ConnectedRadioGroup` and `ConnectedRadio` should rarely, if ever, be used outside of `ConnectedRadioGroupInput`.

## Usage

The components are engineered to be passed into the component prop of <LinkTo component="Organisms/ConnectedForm/ConnectedFormGroup">ConnectedFormGroup</LinkTo>, like so:
