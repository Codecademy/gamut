...
- **Automatic closing**: The PopoverContainer has the `closeOnViewportExit` prop which automatically closes when the Menu when the target row scrolls out of view, even when inside scrollable parent containers
- **Integration with Modals**: You can open Modals from menu items, which will appear above the table header with proper z-index stacking
