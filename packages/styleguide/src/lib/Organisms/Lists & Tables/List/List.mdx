...
- **Header cells**: Use `ListCol` with `columnHeader` or `type="header"` in header rows—see <LinkTo id="Organisms/Lists & Tables/List/ListCol">ListCol</LinkTo> (Header columns).
...
Lists are one of the main design tools for displaying information in a structured, repeated way. They read differently from cards and behave differently from tables.
...
If you have a list with many columns and want to avoid collapsing column content, use the `scrollable` prop. It keeps the row header column pinned on the left and expands every column to at least its minimum flex-basis. Rows and columns scroll together along the horizontal axis so users can compare data and take actions across columns.
