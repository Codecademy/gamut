...
The `scaleInterval` prop determines the interval between scale markers on the x-axis. By default, BarChart automatically calculates an appropriate scale interval based on the `maxRange` value (the scale always starts at 0). However, you can override this with a custom interval to achieve more granular or specific scale markings.
...
The number of tick marks is automatically calculated as: `Math.ceil(maxRange / scaleInterval) + 1`
