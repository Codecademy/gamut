...
- **Accessibility keys** (`gainedNowAt`, `inLabel`, `inOnly`) may be either:
  ...
  - **Function** – Receives scoped context (values, label, unit, locale) and returns the **entire** accessibility summary string. Use for nuanced i18n (pluralization, word order, locale-specific phrasing).

The `translations` prop accepts `PartialBarChartTranslations`: you can override individual keys, and nested `accessibility` and `sortOptions` are merged with defaults at runtime.
...
// Context types for function-based accessibility (exported from @codecademy/gamut):
// BarChartAccessibilityStackedContext: { yLabel, seriesOneValue, seriesTwoValue, unit, locale }
// BarChartAccessibilityInLabelContext:  { yLabel, value, unit, locale }
// BarChartAccessibilityInOnlyContext:   { value, unit, locale }
```
...
The accessibility strings are used in automatically generated `aria-label` attributes for interactive bars and screen reader text for non-interactive bars. You can provide string fragments that are interpolated into the built-in template:
...
For full control over the accessibility summary (e.g. pluralization, word order, locale-specific phrasing), pass functions that receive scoped context and return the **entire** summary string. You can override only the keys you need; others fall back to defaults.
...
<BarChart
  ...
  translations={{
    accessibility: {
      // Stacked bars: full summary from context
      gainedNowAt: (ctx: BarChartAccessibilityStackedContext) =>
        `${ctx.seriesOneValue} ${ctx.unit} gained - now at ${ctx.seriesTwoValue} ${ctx.unit} in ${ctx.yLabel}`,
      // Single bar with label (interactive)
      inLabel: (ctx: BarChartAccessibilityInLabelContext) =>
        `${ctx.value} ${ctx.unit} in ${ctx.yLabel}`.trim(),
      // Single bar without label (non-interactive)
      inOnly: (ctx: BarChartAccessibilityInOnlyContext) =>
...
- Use string accessibility keys when the default template order works; use functions when you need pluralization, different word order, or locale-specific phrasing
