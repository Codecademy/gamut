...
The `modalFocusRef` prop allows you to programmatically control focus on the modal container. This is useful for advanced focus management scenarios where you need to override the default focus behavior (the modal has `data-autofocus` by default).

<Canvas of={ModalStories.FocusManagement} />

Here are examples of how a `Modal` can use the `modalFocusRef` prop to control focus programmatically. This can be useful for accessibility or advanced focus scenarios.

<Canvas of={ModalStories.ConfirmationOnClose} />

<Canvas of={ModalStories.DocumentUploadStyle} />
