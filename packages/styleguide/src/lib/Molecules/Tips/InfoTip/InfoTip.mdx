...
The `InfoTipButton` also has an `aria-roledescription` of `More information button` to provide essential context to screen reader users about the button's purpose.
