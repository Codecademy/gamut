...
- **Tab**: Navigate forward through focusable elements (links, buttons) inside the tip. When reaching the last element, wraps back to the InfoTip button
- **Shift+Tab**: Navigate backward naturally - from the button, exits to the previous page element (no backward trap)
