...
When using the `ToolTip` component, you can use the `id` prop is to associate the tooltip with an interactive element and the interactive element's `aria-describedby` prop must be set to the `id` of the tooltip.
...
When using an icon-only button, a tooltip displaying the label is required to provide context for the button's action. This is accessible directly through the `IconButton`'s required `tip` prop. If you do not provide an `aria-label`, the value of the `tip` prop will be used as the `aria-label` for accessibility purposes. You can adjust positioning and alignment of the tooltip by using the `tipProps` prop.
