...
Using the beak prop, you can add a beak to the popover. This beak can be aligned to the right, left, or center of the popover.

<Canvas of={PopoverStories.Beak} />

Tooltips with a _centered_ beak -`center`- are smaller sized and meant for a single small number of words, like keyboard shortcuts. Similar to the center aligned <LinkTo id="Molecules/Tips/ToolTip">ToolTip</LinkTo>. Please note that `center` aligned popovers can only accept the `center` beak prop.
