...
- **Alert** ([`Alert.stories.tsx`](/story/atoms-feedback-alert--default)): Demonstrates deep controls with default args for `cta`.

- **Coachmark** ([`Coachmark.stories.tsx`](/story/molecules-coachmark--default)): Demonstrates deep controls withcustom argTypes for `popoverProps` with multiple nested properties like `beak`, `position`, `align`, and offsets.

- **FormGroup** ([`FormGroup.stories.tsx`](/story/atoms-form-elements-form-group--default)): Uses the reusable `infotipNestedArgTypes` to expose infotip configuration.
