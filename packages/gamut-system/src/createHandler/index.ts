...
const isDirectional = <T>(
  config: unknown
): config is T & { propName: DirectionalProperty } => {
  return get(config, 'type') === 'directional';
};
...
>(
  ...
  const { dependentProps = [], propName } = config;
  ...
  const styleTemplate: StyleTemplate<
    ThematicProps<Theme, Config>
  > = isDirectional(config)
    ? createDirectionalStyleTemplate(config)
    : createStandardStyleTemplate(config);
