export interface EinConfig {
  ...
  server?: string;
  codex?: string;
  // extensions is just a pass-through, let's not concern ourselves
  // with its contents
  extensions?: {};
}
...
export type ContentItemResourceType =
  | 'book'
  | 'documentation'
  | 'tutorial'
  | 'general_resource';
...
export type BriefProject = ProjectContentItem & {
...
export type LayoutComponent =
  ...
  | NarrativeComponent
  | ImageComponent
  | PersistentCodeEditorComponent
  | ReadOnlyCodeEditorComponent
  | OutputTerminalComponent
  | SQLPreviewComponent
  | TerminalComponent
  | VideoComponent
  | WebBrowserComponent
  | ExpoCodeEditorComponent
...
export type AssessmentFillInTheCode = BaseAssessment & {
  type: 'fill_in_code';
...
export type AssessmentECA = BaseAssessment & {
  type: 'executable_code';
  ein_config: EinConfig;
};

export type AssessmentMultipleChoice = BaseAssessment & {
  type: 'multiple_choice';
  code_snippet: AssessmentCodeSnippet | null;
  answers: AssessmentAnswer[];
  correct_answer: number;
  latex_block: { text: string } | null;
};

export type Assessment =
  | AssessmentECA
  | AssessmentFillInTheCode
...
export type UntestedSubContent = Pick<
  SubContentAssessments,
  'id' | 'slug' | 'title'
>;
