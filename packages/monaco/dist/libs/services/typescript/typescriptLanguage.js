...
export var definition = {
  ...
  tokenizer: {
    ...
    [/[{]/, 'delimiter.bracket', '@bracketCounting'], [/[\[]/, 'delimiter.bracket', '@arrayBracketCounting'], // regular expression: ensure it is terminated before beginning (otherwise it is an operator)
    [/\/(?=([^\\\/]|\\.)+\/([gimsuy]*)(\s*)(\.|;|,|\)|\]|\}|$))/, {
      token: 'regexp',
      bracket: '@open',
      next: '@regexp'
    ...
    [/[;,]/, 'delimiter'], [/[.]/, 'delimiter', '@attribute'], // strings
    {
      include: '@strings'
    }],
    tag: [[/\<\/\w+\>/, {
    ...
    strings: [[/"([^"\\]|\\.)*$/, 'string.invalid'], // non-teminated string
    ...
    [/[{]/, 'delimiter', '@bracketCountingProperty'], // other values
    [/[,]/, {
      token: 'delimiter',
    ...
    functionDefinition: [{
      include: 'whitespace'
    }, [/@validName/, 'definition'], [/[(),]/, 'delimiter'], // matches an equals sign but NOT => in the case of an arrow function
