...
For the actual token values (`fontFamily`, `fontSize`, `fontWeight`, `lineHeight`) and how to reach them in code, see [Design tokens](/reference/design-tokens/). This page covers the design guidance around using them. Gamut uses "Title" to distinguish its visual sizing from semantic heading levels — a `title-md` value doesn't necessarily mean `h2`; see [Text](/components/typography/text/) for how the two compose.
