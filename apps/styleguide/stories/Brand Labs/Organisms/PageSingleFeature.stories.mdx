...
<Meta
  ...
  parameters={{
    subtitle:
      'Single Feature component for landing pages created in Contentful. All props are optional.',
    status: 'updating',
  }}
/>
    ...
    {(args) => (
      <PageSingleFeature
        ...
        media={{
          ...
          alt: 'learning environment demonstration',
        }}
        eyebrow="Codecademy"
      />
    )}
...
When provided with a `linkUrl` images will be able to act as a link.

<Canvas>
  <Story name="Linakble Image">
    {(args) => (
      <PageSingleFeature
        {...args}
        media={{
