...
// inlined from common.mjs — override files can't do relative imports to the lib dir
...
export function resolveDistEntry({
  ...
  if (soft) return null;
  console.error(
    `[NO_DIST] ${pkgName} has no built entry (tried ${cand.join(
      ', '
    )} under ${pkgDir}). ` +
...
export async function bundleToIife({
  ...
  try {
    buildResult = await build({
      ...
      // Suppress CSS extraction: Emotion is CSS-in-JS so no CSS is shipped in
      // _ds_bundle.css — treating CSS imports as empty lets package-build.mjs
      // write the @ds-css-runtime stub that tells the validator this is intentional.
      loader: { '.css': 'empty' },
    });
  } catch (e) {
    ...
    if (siblings.length) {
      console.error(
        `[WORKSPACE_SIBLING] ${siblings.join(
          ', '
      ...
      console.error(
        `[UNRESOLVED_IMPORT] ${unresolved.join(
          ', '
